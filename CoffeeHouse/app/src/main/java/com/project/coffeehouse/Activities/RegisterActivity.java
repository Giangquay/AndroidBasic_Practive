...
import static com.project.coffeehouse.R.*;
...
public class RegisterActivity extends AppCompatActivity {
    ...
    TextInputLayout TXTL_signup_HoVaTen, TXTL_signup_TenDN, TXTL_signup_Email, TXTL_signup_SDT, TXTL_signup_MatKhau, TXTL_signup_reMatKhau;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        TXTL_signup_reMatKhau = (TextInputLayout)findViewById(id.txtl_signup_REMatKhau);
        //endregion
    ...
    private boolean validatePassWord(){
        ...
        String val2 = TXTL_signup_reMatKhau.getEditText().getText().toString().trim();

        if(val.isEmpty()||val2.isEmpty()){
            TXTL_signup_MatKhau.setError(getResources().getString(R.string.not_empty));
            return false;
        }
        else if(!val.equals(val2)){
            TXTL_signup_reMatKhau.setError("Mật khẩu nhập lại không đúng");
            return false;
        }
        else if(!PASSWORD_PATTERN.matcher(val).matches()){
            TXTL_signup_MatKhau.setError("Mật khẩu ít nhất 6 ký tự và không có khoảng trắng!");
            return false;
        }
        else {
            TXTL_signup_MatKhau.setError(null);
            TXTL_signup_MatKhau.setErrorEnabled(false);
            TXTL_signup_reMatKhau.setError(null);
            TXTL_signup_reMatKhau.setErrorEnabled(false);
            return true;
        }
    }
    //endregion
